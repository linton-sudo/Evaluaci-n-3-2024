...
def signup_v1():
    ...
    # Insert default user "linton cherville" / "secret123" into USER_PLAIN if not exists
    try:
        c.execute("INSERT OR IGNORE INTO USER_PLAIN (USERNAME, PASSWORD) VALUES (?, ?)",
                  ("linton cherville", "secret123"))
        conn.commit()
    except Exception:
        pass
...
def signup_v2():
    ...
    # Insert default user "linton cherville" / "secret123" into USER_HASH if not exists
    try:
        default_hash = hashlib.sha256("secret123".encode()).hexdigest()
        c.execute("INSERT OR IGNORE INTO USER_HASH (USERNAME, HASH) VALUES (?, ?)",
                  ("linton cherville", default_hash))
        conn.commit()
    except Exception:
        pass
